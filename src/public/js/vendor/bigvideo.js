	...
	$.BigVideo = function(options) {
		...
		function updateSize() {
			...
			if (containerAspect < mediaAspect) {
				...
				if (currMediaType == 'video') {
					...
					player.height(containerW/mediaAspect);
		...
		BigVideo.init = function() {
			if (!isInitialized) {
				...
				player = $('<video id="'+vidEl.substr(1)+'" class="video-js vjs-default-skin" height="1" width="1" preload="auto" muted="true" data-setup="{}" '+autoPlayString+' webkit-playsinline></video>');
				player.css('position','absolute');
				wrap.append(player);

				var videoTechOrder = ['html5'];
				// var videoTechOrder = ['html5','flash']; // Change 1 : Removed flash. No longer needed
				// If only using mp4s and on firefox, use flash fallback
				/* var ua = navigator.userAgent.toLowerCase();
				var isFirefox = ua.indexOf('firefox') != -1;
				if (settings.useFlashForFirefox && (isFirefox)) {
					videoTechOrder = ['flash', 'html5'];
				} */ // Change 2 : Removed unnecessary functions. No longer needed
