...
onCompleteParams:[]});"video-bg"===R&&a.set($a,{visibility:"hidden",onComplete:m,onCompleteParams:[]})}function G(a,c,d,e,f){var g=e.trans,h=e.transDur,k=a.attr("id"),l,m=!1;0==c?(d=b(".extinct"),l="sven-nothing"):l=d.attr("id");if("no"==e.pursueImage){var n=Ia.find("."+k+" img");l=Ia.find("."+l+" img, ."+ya+" img");ja=n;ya=k}else n=ja,l=b(".sven-not-found");La&&(e.sceneVideo||e.sceneImage&&"no"===e.pursueVideo||"no"===e.pursueVideo)&&(m=!0);f.addLabel(k+"-before");var p,q;p=a.find(".bt-container");
if(0<p.length){var r=p.data().xStart?p.data().xStart:"-100%",t=p.data().fontMax?parseInt(p.data().fontMax):728;p.wrapInner("<div class='bt-wrapper'></div>");q=p.find(".bt-wrapper");q.bigtext({childSelector:"h1",maxfontsize:t,resize:!1});f.set(p,{x:r})}h="default"==h?1.4:parseFloat(h);h/=2;p=b.grep(svenTransitions.vegas,function(a){return a.name==g});0!=c&&b.grep(svenTransitions.vegas,function(a){return a.name==d.data().trans});"fade-screen"==p[0].template?(f.to(Bb,h,{autoAlpha:1,ease:Linear.easeNone}),
...
g=d["outConfig"+c],h="fade-out");f.animTitle=e.animation?e.animation:h;f.animTemplate=e.template?e.template:g.template;"rotator"==f.animTemplate?(g=b.extend({},svenDefaults.rotator,g),d["outConfig"+c]=b.extend({},svenDefaults.rotator,{template:"rotator"})):g=b.extend({},svenDefaults.basic,g);f.pattern=e.pattern?e.pattern:g.pattern;f.ease=e.ease?parseEase(e.ease):g.ease;f.staggerDelay=e.staggerDelay?parseFloat(e.staggerDelay):g.staggerDelay;f.staggerDur=e.staggerDur?parseFloat(e.staggerDur):g.staggerDur;
